...
func _run() -> void:
	...
	# Move next to the pack and fight (auto-target nearest, fire on cooldown).
	var c := Vector2.ZERO
	for e in enemies: c += e.global_position
	c /= maxf(1.0, enemies.size())
	player.global_position = c + Vector2(-60, 0)
		...
		if fire <= 0.0:
			...
			for e in living:
				if player.global_position.distance_to(e.global_position) < player.global_position.distance_to(nearest.global_position):
					nearest = e
			player.facing = (nearest.global_position - player.global_position).normalized()
			player._fire_projectile("prompt_blast", 30)
		if GameManager.state == GameManager.GameState.GAME_OVER or player.hp <= 0:
			break
	_check("button_masher_clears_first_region", cleared)
