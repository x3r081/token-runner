...
## Run from editor or headless to generate procedural textures.
## godot --headless --script scripts/tools/generate_assets.gd
...
func _generate_player_sprites() -> void:
	...
	for i in frames.size():
		var img := _draw_player_frame(i)
		_save_image(img, "player_%s.png" % frames[i])
...
func _generate_tileset() -> void:
	...
	for name in regions:
		...
		var base: Color = regions[name]
		for x in 64:
			for y in 64:
				var n := randf() * 0.08
				var c := base + Color(n, n, n, 0)
				if (x + y) % 8 == 0:
					c = c.lightened(0.05)
				img.set_pixel(x, y, c)
		# Grid lines
		...
		_save_image(img, "tile_%s.png" % name)
...
func _generate_enemies() -> void:
	var enemies := {
		...
		"hallucination": Color(0.9, 0.5, 0.9, 0.7),
...
func _generate_icon() -> void:
	...
	for i in range(44, 84, 6):
		img.set_pixel(i, 74, Color(0.1, 0.1, 0.1))
	_save_image(img, "../icon.png")

func _save_image(img: Image, filename: String) -> void:
	var path := OUT_DIR + filename if not filename.begins_with("../") else "res://assets/textures/" + filename.substr(3)
...
func _draw_hex(img: Image, cx: int, cy: int, r: int, c: Color) -> void:
	for x in img.get_width():
		for y in img.get_height():
			var dx: float = abs(x - cx) / float(r)
			var dy: float = abs(y - cy) / float(r)
			if dx + dy * 1.15 <= 1.0:
				img.set_pixel(x, y, c)
